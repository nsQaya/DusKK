...
using TDV.Migrations.Seed.Host;
...
namespace TDV.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : TDVAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<TDVDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{TDVConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{TDVConsts.ConnectionStringName}", constring);
        }
